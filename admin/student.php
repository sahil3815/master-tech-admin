    ...
    <div class="main-body">
          ...
          <nav aria-label="breadcrumb" class="main-breadcrumb">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item"><a href="fetch_student">Students</a></li>
          ...
          <div class="row gutters-sm">
                  ...
                  <div class="d-flex flex-column align-items-center text-center">
                    ...
                    <div class="mt-3">
                      ...
                      <p id="status-text" class="text-muted font-size-sm"><?php echo $studnt['status']; ?></p>
                        ...
                        <button id="sendEmailBtn" class="btn btn-info" data-bs-toggle="tooltip" data-bs-placement="top" title="Send Profile Email">
            ...
            <div class="row gutters-sm">
                <div class="col-md-12">
                    <div class="card mb-3" id="payment-history-section">
                        <div class="card-body">
                            <h5 class="card-title">Payment History</h5>
                            <?php if (count($payments) > 0): // Check if there are payments ?>
                            ...
                            <?php else: ?>
                                <!-- If no payment history, show a message instead of the table -->
...
<script>
  $(document).ready(function() {
            ...
            if (deleteConfirmed) {
                ...
                .done(function(response) {
                    ...
                    if (response.trim() === 'success') {
                        ...
                        window.location.href = 'fetch_student';
                    } else {
    ...
    $(document).ready(function() {
        ...
        $(document).ready(function() {
            $('#feesForm').submit(function(event) {
                ...
                // Send data to update_payment.php first to update the payment
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // If payment update is successful, send data to send_pay_mail.php for email and PDF
                        if (data.success) {
                            // Send the same data to send_pay_mail.php
                            $.ajax({
                                url: 'mail/send_pay_mail.php',  // URL of your mail sending script
                                type: 'POST',
                                data: formData,
                                success: function(mailResponse) {
                                    var mailData = JSON.parse(mailResponse);
                                    if (mailData.success) {
                                        alert('Payment updated successfully and email sent!');
                                        $('#feesModal').modal('hide');
                                        location.reload();  // Reload page to show updated data
                                    } else {
                                        alert('Error in sending email: ' + mailData.message);
                                    }
                                },
                                error: function(xhr, status, error) {
    ...
    $(document).ready(function() {
        // Check if the table exists (i.e., there is at least one row)
        if ($('#paymentHistoryTable').length > 0 && $('#paymentHistoryTable tbody tr').length > 0) {
            // Initialize DataTable for payment history
            $('#paymentHistoryTable').DataTable();
        } else {
            // If no records, we do not initialize DataTable, and display the "No record found" message
            $('#paymentHistoryTable').hide();  // Hide the table (if any, but it shouldn't be here)
        }
    });
...
$(document).ready(function() {
    // Click event for the "Send Profile" button
    $('#sendEmailBtn').click(function() {
        var studentId = '<?php echo $studnt["id"]; ?>'; // Get student ID dynamically
        $.ajax({
            url: 'mail/index.php', // PHP file to handle sending the email
            type: 'POST',
            data: { stu_id: studentId }, // Pass the student ID to the PHP script
            success: function(response) {
                alert(response); // Show success message
            },
            error: function(xhr, status, error) {
                alert("Error sending email: " + error); // Show error message
            }
        });
    });
});
